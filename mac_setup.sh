...
# Provide a default color flag if not already defined by the environment/shell rc
...
# Resilient update alias (skips brew steps if brew not present)
alias update='sudo softwareupdate -i -a; command -v brew >/dev/null 2>&1 && { brew update; brew upgrade; brew cleanup; }'
...
###
# Guarded MySQL legacy symlink creation
if [ -x /opt/homebrew/opt/mysql-client@5.7/bin/mysql ]; then
